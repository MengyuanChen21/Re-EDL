...
def get_dataset(dataset_name, batch_size, split=[.8, .2], seed=1, test_shuffle_seed=None, batch_size_eval=1024,
    ...
    if dataset_name == 'segment_window_sky_missing':
        ...
        dataset = Dataset.emnist(train=True)
        test_set = Dataset.emnist(train=False)
        output_dim = 26
    elif dataset_name == 'CIFAR10':
        dataset = Dataset.cifar10(train=True, image_transforms=[tvt.RandomHorizontalFlip(),
                                                                tvt.RandomCrop(32, 4),
        ...
        dataset = Dataset.dtd(train=True, image_transforms=[tvt.Resize((32, 32))])
        test_set = Dataset.dtd(train=False, image_transforms=[tvt.Resize((32, 32))])
        output_dim = 47
    elif dataset_name == 'GTSRB':
        dataset = Dataset.GTSRB(train=True, image_transforms=[tvt.Resize((32, 32))])
        test_set = Dataset.GTSRB(train=False, image_transforms=[tvt.Resize((32, 32))])
        output_dim = 50
    elif dataset_name == 'OxfordIIITPet':
        dataset = Dataset.OxfordIIITPet(train=True, image_transforms=[tvt.Resize((32, 32))])
        test_set = Dataset.OxfordIIITPet(train=False, image_transforms=[tvt.Resize((32, 32))])
        output_dim = 37
    elif dataset_name == 'FGVCAircraft':
        dataset = Dataset.FGVCAircraft(train=True, image_transforms=[tvt.Resize((32, 32))])
        test_set = Dataset.FGVCAircraft(train=False, image_transforms=[tvt.Resize((32, 32))])
        output_dim = 100
    elif dataset_name == 'SVHN_oodom':
        dataset = Dataset.svhn(train=True, tensor_transforms=[tvt.Lambda(lambda x: x * 255.)])
        test_set = Dataset.svhn(train=False, tensor_transforms=[tvt.Lambda(lambda x: x * 255.)])
        ...
        dataset = Dataset.lsun(train=True)
        test_set = Dataset.lsun(train=False)
        output_dim = 10
    elif dataset_name == 'FAKE':
        dataset = Dataset.fake(train=True)
        test_set = Dataset.fake(train=False)
        ...
        dataset = Dataset.LFWPeople(train=True, image_transforms=[tvt.Resize((32, 32))])
        test_set = Dataset.LFWPeople(train=False, image_transforms=[tvt.Resize((32, 32))])
        output_dim = 5749  # ?
    elif dataset_name == 'Places365':
        dataset = Dataset.Places365(image_transforms=[tvt.Resize((32, 32))])
        test_set = Dataset.Places365(image_transforms=[tvt.Resize((32, 32))])
        output_dim = 5749  # ?
    elif dataset_name == 'Flowers102':
        dataset = Dataset.Flowers102(image_transforms=[tvt.Resize((32, 32))])
        test_set = Dataset.Flowers102(image_transforms=[tvt.Resize((32, 32))])
        output_dim = 211  # ?
    elif dataset_name == 'Food101':
        dataset = Dataset.Food101(image_transforms=[tvt.Resize((32, 32))])
        test_set = Dataset.Food101(image_transforms=[tvt.Resize((32, 32))])
        output_dim = 101  # ?
...
class Dataset:
    ...
    * All features are normalized to have zero mean and unit variance.
    ...
    @classmethod
    def emnist(cls, train=True, image_transforms=[], tensor_transforms=[]):
        """
        Returns the KMNIST dataset, either for training or testing:
        * Task: classification
        * Features: [1, 28, 28]
    ...
    def fake(cls, train=True, image_transforms=[], tensor_transforms=[]):
        ...
        * Samples: 100 000
        """
        return tvd.FakeData(size=10000, image_size=(3, 32, 32), num_classes=10,
                            transform=tvt.Compose(image_transforms + [tvt.ToTensor()] + tensor_transforms))
    ...
    @classmethod
    def dtd(cls, train=True, image_transforms=[], tensor_transforms=[]):
        split = 'train' if train else 'test'
        return tvd.DTD(
            DATASET_DIR, download=True, split=split,
            transform=tvt.Compose(image_transforms + [tvt.ToTensor()] + tensor_transforms)
    ...
    @classmethod
    def GTSRB(cls, train=True, image_transforms=[], tensor_transforms=[]):
        split = 'train' if train else 'test'
        return tvd.GTSRB(
            DATASET_DIR, download=True, split=split,
            transform=tvt.Compose(image_transforms + [tvt.ToTensor()] + tensor_transforms)
    ...
    @classmethod
    def OxfordIIITPet(cls, train=True, image_transforms=[], tensor_transforms=[]):
        split = 'trainval' if train else 'test'
        return tvd.OxfordIIITPet(
            DATASET_DIR, download=True, split=split,
            transform=tvt.Compose(image_transforms + [tvt.ToTensor()] + tensor_transforms)
        )

    @classmethod
    def FGVCAircraft(cls, train=True, image_transforms=[], tensor_transforms=[]):
        split = 'train' if train else 'test'
        return tvd.FGVCAircraft(
            DATASET_DIR, download=True, split=split,
            transform=tvt.Compose(image_transforms + [tvt.ToTensor()] + tensor_transforms)
        )

    @classmethod
    def LFWPeople(cls, train=True, image_transforms=[], tensor_transforms=[]):
        # split = 'train' if train else 'test'
        split = 'test'  # only need to download the test split
        return tvd.LFWPeople(
            DATASET_DIR, download=True, split=split,
            transform=tvt.Compose(image_transforms + [tvt.ToTensor()] + tensor_transforms)
        )

    @classmethod
    def Places365(cls, train=True, image_transforms=[], tensor_transforms=[]):
        # split = 'train' if train else 'test'
        split = 'val'  # only need to download the val split
        return tvd.Places365(
            DATASET_DIR, download=False, split=split, small=True,
            transform=tvt.Compose(image_transforms + [tvt.ToTensor()] + tensor_transforms)
        )

    @classmethod
    def Flowers102(cls, train=True, image_transforms=[], tensor_transforms=[]):
        # split = 'train' if train else 'test'
        split = 'test'  # only need to download the test split
        return tvd.Flowers102(
            DATASET_DIR, download=True, split=split,
            transform=tvt.Compose(image_transforms + [tvt.ToTensor()] + tensor_transforms)
        )

    @classmethod
    def Food101(cls, train=True, image_transforms=[], tensor_transforms=[]):
        # split = 'train' if train else 'test'
        split = 'test'  # only need to download the test split
        return tvd.Food101(
            DATASET_DIR, download=True, split=split,
